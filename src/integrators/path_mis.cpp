...
public:
    ...
    std::pair<Spectrum, Bool> sample(const Scene *scene,
        ...
        // [MIS] do multiple loop
        // TODO: take care of sample contribution such as in direct MIS (weight)
        // TODO: how to manage only BSDF or Emitter sampling when path tracing?
        // Only manage weight when sum up?
        ...
        // ScalarFloat sample_weight = 1.f / (ScalarFloat) batch_samples;

        // outside variables
        Mask valid_ray;
        Spectrum result_sum = 0.f;
        ...
        valid_ray                     = !m_hide_emitters && dr::neq(scene->environment(), nullptr);
        ...
        while (loop(active)) {
            ...
            if (dr::any_or<true>(dr::neq(si.emitter(scene), nullptr))) {
                ...
                Float mis_bsdf = mis_bsdf = mis->mis_weight(alpha_bsdf, prev_bsdf_pdf, em_pdf);

                auto bsdf_val = ds.emitter->eval(si, prev_bsdf_pdf > 0.f);
                
                // update only at first bounce
                if (depth <= 1) {
                    mis->add_sampling_data(0, bsdf_val, {prev_bsdf_pdf, em_pdf});
            ...
            if (dr::any_or<true>(active_em)) {
                ...
                Float mis_em = dr::select(ds.delta, 1.f, mis->mis_weight(alpha_emitter, ds.pdf, bsdf_pdf));

                // [MIS] track information only at first bounce
                if (depth <= 0) {

                    mis->add_sampling_data(1, bsdf_val * em_weight, {bsdf_pdf, ds.pdf});
                    // mis->update_n_samples();
                }
                // else {
                //     mis_em = dr::select(ds.delta, 1.f, mis_weight(ds.pdf, bsdf_pdf));
                // }
    ...
    // }
        ...
        // [MIS]: update alphas depending of number of samples
        if (mis->number_of_samples() % batch_samples == 0)
            mis->update_alphas();

        return {
            /* spec  = */ dr::select(valid_ray, result_sum, 0.f),
            /* valid = */ valid_ray
        };
    }
